          ...
          {scorePercentage && (
            <div className="absolute bottom-5 left-5 right-5 z-20 flex items-end justify-between">
              <div className="flex flex-col">
                <span className="font-label-caps text-[8px] text-secondary/60 tracking-[0.15em] uppercase mb-0.5">
                  {scoreLabel ||
                    (type === "service"
                      ? "SLA Quality Index"
                      : "System Efficiency")}
                </span>
                <span className="font-headline text-[22px] font-bold text-accent leading-none">
                  {scorePercentage}
                  <span className="text-accent text-[14px]">%</span>
                </span>
              </div>
              {/* Minimalist Progress Line */}
              <div className="w-[70px] h-[2px] bg-secondary/15 overflow-hidden mb-1 rounded-full">
                <motion.div
            ...
            {features && features.length > 0 && (
              <div className="flex flex-wrap gap-1.5 mb-5">
                {features.slice(0, 4).map((feature, idx) => (
                  <span
                    key={idx}
                    className="font-label-caps text-[8.5px] uppercase tracking-[0.08em] px-2.5 py-1 border border-outline/30 bg-surface-container-lowest text-secondary/80 hover:border-accent hover:text-accent transition-colors duration-300 cursor-default"
                  >
                    {feature}
                  </span>
                ))}
              ...
              <div className="grid grid-cols-2 md:grid-cols-4 gap-4 pt-4 border-t border-outline/20">
                {stats.slice(0, 4).map((stat, idx) => (
                  <div key={idx} className="flex flex-col">
                    <span className="font-label-caps text-[8px] text-secondary/40 tracking-[0.12em] uppercase mb-0.5">
                      {stat.label}
                    </span>
                    <span className="font-headline text-[13px] font-bold text-secondary">
                      {stat.value}
                    </span>
                  </div>
                ))}
                  ...
                  <table className="w-full text-left border-collapse min-w-[600px]">
                    <thead>
                      <tr className="bg-surface text-secondary font-label-caps text-[8.5px] tracking-[0.1em] uppercase border-b border-outline/20">
                        <th className="py-2.5 px-3.5 w-[25%]">
                          Service Parameter
                        </th>
                        <th className="py-2.5 px-3.5 w-[25%]">
                          Bronze Standard
                        ...
                        <th className="py-2.5 px-3.5 w-[25%] text-accent">
                          Gold Enterprise
                        </th>
                      </tr>
                    </thead>
                    <tbody>
                      {displaySlaPlans.map((plan, idx) => (
                        <tr
                          key={idx}
                          className="border-b border-outline/10 last:border-0 hover:bg-surface transition-colors"
                        >
                          <td className="py-2.5 px-3.5 font-headline text-[11.5px] font-semibold text-secondary">
                            {plan.parameter}
                          </td>
                          ...
                          <td className="py-2.5 px-3.5 font-body text-[11.5px] font-medium text-accent bg-accent/[0.03]">
                            {plan.gold}
                          </td>
                        </tr>
                      ))}
                      ...
                      {displayModels.map((model, idx) => {
                        const h =
                          typeof model.height === "number"
                            ? unitSystem === "metric"
                              ? `${model.height} mm`
                              : `${(model.height / 25.4).toFixed(1)} in`
                            : model.height;
                        const w =
                          typeof model.width === "number"
                            ? unitSystem === "metric"
                              ? `${model.width} mm`
                              : `${(model.width / 25.4).toFixed(1)} in`
                            : model.width;
                        const d =
                          typeof model.depth === "number"
                            ? unitSystem === "metric"
                              ? `${model.depth} mm`
                              : `${(model.depth / 25.4).toFixed(1)} in`
                            : model.depth;
                        return (
                          <tr
                            key={idx}
                            className="border-b border-outline/10 last:border-0 hover:bg-surface transition-colors"
                          >
                            <td className="py-2.5 px-3.5 font-headline text-[11.5px] font-semibold text-accent">
                              {model.name}
                            </td>
                            <td className="py-2.5 px-3.5 font-body text-[11.5px] text-secondary/70">
                              {model.capacity || "N/A"}
                            ...
                            <td className="py-2.5 px-3.5 font-body text-[11.5px] text-secondary/70">
                              {model.inputVoltage || model.voltage || "N/A"}
                            </td>
                            <td className="py-2.5 px-3.5 font-body text-[11.5px] text-secondary/70">
                              {model.outputVoltage || model.voltage || "N/A"}
                            </td>
                            <td className="py-2.5 px-3.5 font-body text-[11.5px] text-secondary/70">
                              {model.efficiency}
                            </td>
