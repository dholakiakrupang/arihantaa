      ...
      <div className="max-w-[1440px] mx-auto">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 border-t border-x border-outline-variant/30">
          {STATS_DATA.map((stat, i) => (
            <div 
              key={i} 
              className="md:border-r border-b md:border-b-0 last:border-b-0 border-outline-variant/30 md:last:border-r-0 flex flex-col justify-between p-6 sm:p-7 md:p-8 lg:p-12 hover:bg-accent/[0.01] transition-all duration-500 group"
            >
              {/* Big Animated Value */}
              <div>
                <StatCard
                ...
                {/* Subtitle / Desc */}
                <p className="font-body text-[12px] sm:text-[13px] text-secondary/70 leading-relaxed mt-2.5 sm:mt-3 lg:mt-4 max-w-xs group-hover:text-on-surface transition-colors duration-300">
                  {stat.desc}
                </p>
              </div>

              {/* Accent Line Indicator */}
              <div className="w-full h-px bg-outline-variant/20 mt-4 sm:mt-5 lg:mt-8 overflow-hidden relative">
